...
@section('content')
    ...
    <section class="banner-section overlay banner-section-blogs">
        <div class="banner-inner">
            <h1 class="page-title">@lang('menu.SpecialBlog')</h1>
            <nav class="mt-70" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('homepage', app()->getLocale()) }}">@lang('menu.Home')</a></li>
                    <li class="breadcrumb-item active" aria-current="page">@lang('menu.SpecialBlog')</li>
                </ol>
            </nav>
        </div>
    </section>
                    ...
                    @if(count($totalRecords)>0)
                        <div class="row">
                            @foreach($totalRecords as $blogAll)
                                <div class="col-lg-6">
                                    <div class="blog-single-item style-three wow  delay-0-6s slow animated" style="visibility: visible; animation-name: ;">
                                        ...
                                        <div class="blog-content">
                                            ...
                                            <div class="content-bottom">
                                                <h6><a href="{{ route('SpecialBlog', ['id'=>$blogAll->id, 'locale'=>app()->getLocale()]) }}" class="black_text"> {{ $blogAll->{'title_'.app()->getLocale()} }} </a></h6>

                                                <div class="by-views">
                                                    <span class="admin black_text"><b class="gray_text">BY:</b> {{ $blogAll->author->{'name_'.app()->getLocale()} }}</span>
                                                    <span class="love"><i class="fas fa-eye gray_text"></i>{{ $blogAll->view }}</span>
                                                    <span class="admin black_text"><b class="gray_text">Category:</b> {{ $blogAll->getCategory->{'name_'.app()->getLocale()} }}</span>
                                                </div>

                                                <p>{{ $blogAll->{'short_'.app()->getLocale()} }}</p>
                                                <a class="read-more" href="{{ route('SpecialBlog', ['id'=>$blogAll->id, 'locale'=>app()->getLocale()]) }}">Read More</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <div class="style-btn-all">
                                    <a href="{{ route('SpecialBlog', app()->getLocale()) }}" class="theme-btn wow style-three-btn delay-0-2s slow">@lang('common.see_all')</a>
                                </div>
                            </div>
                        </div>
                    @endif
